//
//  shortest_word_ladder.c
//  PP5_Shortest_Word_Ladder
//
//  Created by Conor McJannett
//
...
int countWordsOfLength(char* filename, int wordSize) {
    ...
    int count = 0;
    char word[30];
    while (!feof(file)) {  // read all words from file
        fscanf(file, "%s", word);
        if (strlen(word) == wordSize) {  // if word is size wordSize
            count++;
        }
    }
    fclose(file);
...
bool buildWordArray(char* filename, char** words, int numWords, int wordSize) {
    ...
    char newWord[30];
    while (!feof(file)) {  // read all words from file
        fscanf(file, "%s", newWord);
        if (strlen(newWord) == wordSize) {  // if word is size wordSize
            wordCount++;  // count word
            if (wordCount > numWords) {  // check if wordCount exceeded numWords
                break;
            }
            strcpy(words[wordCount-1], newWord);
        }
    }
    fclose(file);
...
int findWord(char** words, char* aWord, int loInd, int hiInd) {
    ...
    //---------------------------------------------------------
    if (hiInd < loInd || loInd < 0) return -1;  // base case check for invalid bounds
    int mid = ((hiInd - loInd) / 2) + loInd;
    if (strcmp(aWord, words[mid]) < 0) return findWord(words, aWord, loInd, mid-1);
    if (strcmp(aWord, words[mid]) > 0) return findWord(words, aWord, mid+1, hiInd);
    return (strcmp(aWord, words[mid]) == 0 ? mid : -1);
}
...
void insertWordAtFront(WordNode** ladder, char* newWord) {
    ...
    //---------------------------------------------------------
    if (ladder == NULL) {
        printf("ERROR[insertWordAtFront]: INVALID POINTER TO LADDER POINTER\n");
        return;
    }
...
WordNode* copyLadder(WordNode* ladder) {
    ...
    WordNode* cpy = (WordNode*)malloc(sizeof(WordNode));
    cpy->myWord = ladder->myWord;
    cpy->next = copyLadder(ladder->next);
    return cpy;
}
...
WordNode* findShortestWordLadder(   char** words,
    ...
    WordNode* shortestLadder = NULL;
    char myWord[wordSize + 1];
    int index = findWord(words, startWord, 0, numWords - 1);
    usedWord[index] = true;
    insertWordAtFront(&myLadder, words[index]);
    insertLadderAtBack(&myList, myLadder);
    while (myList != NULL) {
        myLadder = popLadderFromFront(&myList);
        for (int i = 0; i < wordSize; i++) {
            strcpy(myWord, myLadder->myWord);
            for (int j = 0; j < 26; j++) {
                myWord[i] = 'a' + j;
                index = findWord(words, myWord, 0, numWords - 1);
                if (index > -1) {
                    if (!usedWord[index] || strcmp(words[index], finalWord) == 0) {
                        usedWord[index] = true;
                        if (strcmp(words[index], finalWord) == 0) {
                            insertWordAtFront(&myLadder, finalWord);
                            if (getLadderHeight(shortestLadder) > getLadderHeight(myLadder) || getLadderHeight(shortestLadder) == 0) {
                                freeLadder(shortestLadder);
                                shortestLadder = NULL;
                                shortestLadder = copyLadder(myLadder);
                            }
                        } else {
                            anotherLadder = copyLadder(myLadder);
